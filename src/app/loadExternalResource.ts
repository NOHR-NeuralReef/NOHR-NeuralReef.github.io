...
// Main export function to load only dynamic resources not in index.html
export function loadExternalResource(url: string): Promise<string> {
    // Check if resource is already loaded by index.html
    const isPreloaded = document.querySelector(`link[href="${url}"], script[src="${url}"]`);
    if (isPreloaded) {
